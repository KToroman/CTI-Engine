...
class App(AppRequestsInterface):
    def __init__(self, shutdown_event: SyncEvent, passive_mode_event: SyncEvent, load_event: SyncEvent,
                 ...
                 status_queue: Queue, project_queue: Queue, cancel_event: SyncEvent, restart_event: SyncEvent,
                 model: Model):
        # Events:
        ...
        self.queue_list.extend([self.load_path_queue, self.__source_file_name_queue, self.__error_queue,
                                self.status_queue, self.__project_queue, self.__hierarchy_fetcher_work_queue,
                                self.__file_saver_work_queue, self.__pid_queue, self.source_file_queue])
        ...
        self.hierarchy_fetcher = HierarchyFetcher(self.__hierarchy_fetching_event,
                                                  self.shutdown_event, self.source_file_queue, self.__pid_queue,
                                                  max_workers=50)

    def prepare_threads(self):
        self.hierarchy_process: HierarchyProcess = HierarchyProcess(self.hierarchy_process_shutdown,
                                                                    self.hierarchy_fetcher,
                                                                    self.__error_queue,
                                                                    self.__hierarchy_fetcher_work_queue,
                                                                    self.__hierarchy_fetching_event)
        self.hierarchy_thread: HierarchyThread = HierarchyThread(self.shutdown_event, self.__fetching_hierarchy,
                                                                 self.source_file_queue, self.__model,
                                                                 self.__model_lock, self.hierarchy_process,
                                                                 self.__hierarchy_fetcher_work_queue,
